...
bool check = FALSE;
#else
#define vertices (chull->vertices)
#define edges (chull->edges)
#define faces (chull->faces)
#define debug (chull->debug)
#define check (chull->check)
#endif
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
tVertex	MakeNullVertex( tConvexHull chull )
{
    tVertex  v;
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	ReadVertices( tConvexHull chull )
{
    tVertex  v;
    int      x, y, z;
    int	    vnum = 0;
...
void    DoubleTriangle( tConvexHull chull )
{
    ...
    f0 = MakeFace( chull, v0, v1, v2, f1 );
    f1 = MakeFace( chull, v2, v1, v0, f0 );
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	ConstructHull( tConvexHull chull )
{
    tVertex  v, vnext;
    int 	    vol;
    ...
    do {
        ...
        if ( !v->mark ) {
            v->mark = PROCESSED;
            changed = AddOne( chull, v );
            CleanUp( chull, &vnext ); /* Pass down vnext in case it gets deleted. */
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
bool 	AddOne( tConvexHull chull, tVertex p )
{
    tFace  f;
    tEdge  e, temp;
    int 	  vol;
    ...
    do {
        ...
        /* e border: make a new face. */
            e->newface = MakeConeFace( chull, e, p );
        e = temp;
    } while ( e != edges );
    return TRUE;
}
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
int  VolumeSign( tConvexHull chull, tFace f, tVertex p )
{
    float  vol;
    int    voli;
    ...
    if (fabs(vol) < FLT_EPSILON) return 0;
...
/*-------------------------------------------------------------------*/
void	PrintPoint( tConvexHull chull, tVertex p )
{
    int	i;
...
/*---------------------------------------------------------------------
 ...
 the new face.
 ---------------------------------------------------------------------*/
tFace	MakeConeFace( tConvexHull chull, tEdge e, tVertex p )
{
    tEdge  new_edge[2];
    tFace  new_face;
    int 	  i, j;
    ...
    /* Make the new face. */
    new_face = MakeNullFace(chull);
    new_face->edge[0] = e;
    new_face->edge[1] = new_edge[0];
    new_face->edge[2] = new_edge[1];
    MakeCcw( chull, new_face, e, p );
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	MakeCcw( tConvexHull chull, tFace f, tEdge e, tVertex p )
{
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
tEdge 	MakeNullEdge( tConvexHull chull )
{
    tEdge  e;
...
/*--------------------------------------------------------------------
 ...
 to the empty cell.
 ---------------------------------------------------------------------*/
tFace 	MakeNullFace( tConvexHull chull )
{
    tFace  f;
    int    i;
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
tFace   MakeFace( tConvexHull chull, tVertex v0, tVertex v1, tVertex v2, tFace fold )
{
    tFace  f;
    tEdge  e0, e1, e2;
    ...
    if( !fold ) {
        e0 = MakeNullEdge(chull);
        e1 = MakeNullEdge(chull);
        e2 = MakeNullEdge(chull);
    }
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	CleanUp( tConvexHull chull, tVertex *pvnext )
{
    CleanEdges(chull);
    CleanFaces(chull);
    CleanVertices( chull, pvnext );
}
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	CleanEdges( tConvexHull chull )
{
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	CleanFaces( tConvexHull chull )
{
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	CleanVertices( tConvexHull chull, tVertex *pvnext )
{
    tEdge    e;
    tVertex  v, t;
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	Consistency( tConvexHull chull )
{
    register tEdge  e;
    register int    i, j;
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	Convexity( tConvexHull chull )
{
    register tFace    f;
    register tVertex  v;
    int               vol;
...
/*---------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------*/
void	CheckEuler( tConvexHull chull, int V, int E, int F )
{
    if ( check )
...
/*===================================================================
 ...
 =====================================================================*/
/*-------------------------------------------------------------------*/
void	PrintOut( tConvexHull chull, tVertex v )
{
    fprintf( stderr, "\nHead vertex %d = %6x :\n", v->vnum, v );
    PrintVertices(chull);
    PrintEdges(chull);
...
/*-------------------------------------------------------------------*/
void	PrintVertices( tConvexHull chull )
{
    tVertex  temp;
...
/*-------------------------------------------------------------------*/
void	PrintEdges( tConvexHull chull )
{
    tEdge  temp;
    int 	  i;
...
/*-------------------------------------------------------------------*/
void	PrintFaces( tConvexHull chull )
{
    int 	  i;
    tFace  temp;
...
/*-------------------------------------------------------------------
 ...
 the face.
 -------------------------------------------------------------------*/
void	CheckEndpts ( tConvexHull chull )
{
    int 	   i;
    tFace   fstart;
    tEdge   e;
...
/*------------------------------------------------------------------
 ...
 ------------------------------------------------------------------*/
void    EdgeOrderOnFaces ( tConvexHull chull ) {
    tFace f = faces;
    tEdge new;
    int i,j;
